    ...
    return (
        ...
        <View style={styles.contP}>
                ...
                <View style={[styles.contEv, {borderColor: pokeTheme[type]}]}>
                    <Text style={[styles.poke1, {color: pokeTheme[type]}]}>{PokeBridge?.chain?.species?.name}</Text>
                </View>
                <FontAwesome5 name='arrow-right' style={[styles.iconEv, {color: pokeTheme[type]}]}/>
                <Text style={styles.tEvol}>Lvl {PokeBridge?.chain?.evolves_to[0]?.evolution_details[0]?.min_level}</Text>
                <View style={[styles.contEv, {borderColor: pokeTheme[type]}]}>
                    <Text style={[styles.poke1, {color: pokeTheme[type]}]}>{PokeBridge?.chain?.evolves_to[0]?.species.name}</Text>
                </View>
                ...
                <Text style={styles.tEvol2}>Lvl {PokeBridge?.chain?.evolves_to[0]?.evolves_to[0]?.evolution_details[0]?.min_level}</Text>
                <View style={[styles.contEv2, {borderColor: pokeTheme[type]}]}>
                    <Text style={[styles.poke2, {color: pokeTheme[type]}]}>{PokeBridge?.chain?.evolves_to[0]?.evolves_to[0]?.species.name}</Text>
                </View>
                <Image 
